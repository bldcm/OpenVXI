  ...
  * in the middle of SSML documents.  These recordings are passed by the
  ...
  * identifiers (VXIStrings) and their associated recording (VXIContent).
  * Each identifier is proceeded by PROMPT_AUDIO_REFS_SCHEME.
  *
  * Withing the SSML document, the audio recording is replaced by a mark
  * element whose name attribute is an identifier inside the PROMPT_AUDIO_REFS
  * map.
  */
 ...
 #define PROMPT_PREFETCH_REQUEST      L"vxi.prompt.prefetchLevel" /* VXIInteger, 0 if no prefetch is requested, 1 is prefetch is desired */
 ...
 */
 ...
 typedef struct VXIpromptInterface {
   ...
   /**
    * @name Play
    * @memo Start playing queued segments, non-blocking
    *
    * @doc
   ...
   /**
    * @name PlayFiller
    * @memo Queues and possibly starts the special play of a filler segment, 
    *       non-blocking
    * @doc
    ...
    * @param src         [IN] URI or platform dependant path to the content;
    ...
    *                    SSML (type set to VXI_MIME_SSML) or simple wchar_t
    *                    text (type set to VXI_MIME_UNICODE_TEXT).   The
    *                    implementation also may support other formats.
    * @param properties  [IN] Properties to control the fetch, queue, and
    *                    play, as specified above. May be NULL.
    *
    * @return VXIprompt_RESULT_SUCCESS on success
